...
async def _process(gpt_data: dict, req_dict: dict, export_pdf: bool) -> PatternResponse:
    ...
    validation = validator.validate(
        rows=rows, finishing=finishing, craft_type="embroidery",
    ...
    raw_instructions = await polish_instructions(gpt_data, "embroidery", req_dict.get("skill_level", "beginner"))

    pattern = GeneratedPattern(
        title=gpt_data.get("title", "Embroidery Pattern"),
        description=gpt_data.get("description", ""),
        craft_type=CraftType.embroidery,
...
@router.post("/generate", response_model=PatternResponse, summary="Generate embroidery pattern from text prompt")
async def generate(req: PatternRequest):
    try:
        gpt_data = await generate_from_prompt(
            prompt=req.prompt, craft_type="embroidery",
...
@router.post("/generate-from-image", response_model=PatternResponse, summary="Generate embroidery pattern from image")
async def generate_from_image_endpoint(
    ...
    try:
        ...
        gpt_data = await generate_from_image(
            ...
            craft_type="embroidery", skill_level=skill_level,
