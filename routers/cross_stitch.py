...
router = APIRouter(prefix="/cross_stitch", tags=["Cross Stitch"])
...
async def _process(gpt_data: dict, req_dict: dict, export_pdf: bool) -> PatternResponse:
    ...
    validation = validator.validate(
        rows=rows, finishing=finishing, craft_type="cross_stitch",
    ...
    raw_instructions = await polish_instructions(gpt_data, "cross_stitch", req_dict.get("skill_level", "beginner"))

    pattern = GeneratedPattern(
        title=gpt_data.get("title", "Cross Stitch Pattern"),
        description=gpt_data.get("description", ""),
        craft_type=CraftType.cross_stitch,
...
@router.post("/generate", response_model=PatternResponse, summary="Generate cross stitch pattern from text prompt")
async def generate(req: PatternRequest):
    try:
        gpt_data = await generate_from_prompt(
            prompt=req.prompt, craft_type="cross_stitch",
...
@router.post("/generate-from-image", response_model=PatternResponse, summary="Generate cross stitch pattern from image")
async def generate_from_image_endpoint(
    ...
    try:
        ...
        gpt_data = await generate_from_image(
            ...
            craft_type="cross_stitch", skill_level=skill_level,
